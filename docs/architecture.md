...
### Scope: mirrors AND the primary (update-safe)

Remote-Control enforcement and session-propagation ship on the per-instance mirror clones (`lib/asar-patch-clone.sh --mode=mirror`, invoked by `build-clone-app.sh`) **and** on the primary `/Applications/Claude.app` (`--mode=primary`, invoked by `install` / `repair` / `primary-patch`). The two modes differ in exactly two ways:

- **Auto-updates.** The mirror plist carries `disableAutoUpdates=YES` (a mirror is not Anthropic-signed, so a Squirrel check would fail DR verification — we silence it). The primary plist OMITS that key — the primary MUST keep receiving updates.
- **Re-sign.** A mirror is ad-hoc re-signed wholesale by `build-clone-app.sh`. The primary is re-signed **in place, inside-out, ad-hoc, with an explicit LOOSE Designated Requirement** — `designated => anchor apple generic and identifier "com.anthropic.claudefordesktop"` — that has no team-id and no cdhash pin. An Anthropic-signed update satisfies that DR, so Squirrel keeps applying updates while the app stays patched. Signing is inside-out (each nested Mach-O bundle — Squirrel/Electron frameworks, every Helper `.app` — first, then the outer app last, each with `--options runtime` + preserved entitlements), NEVER `--deep` (which leaves "nested code is modified or invalid" under ad-hoc).

An earlier iteration re-signed the primary ad-hoc with `--deep` and no explicit requirement, pinning the DR to a per-build cdhash; Anthropic's next update no longer satisfied it, Squirrel refused to apply it, and the primary froze. The loose DR + inside-out signing is the fix. `primary-patch` snapshots the pristine `app.asar` + `app.asar.unpacked` first so `primary-unpatch` can restore the FULL bundle (the native `.node` modules are never lost). The `com.user.claude-primary-patch-refresh` launchd agent re-applies the patch after each Squirrel drop.
...
2. **`com.user.claude-clone-refresh`** — `WatchPaths` on `/Applications/Claude.app/Contents/Info.plist`. Squirrel writes to this file at update-end; on mtime change the agent invokes `bin/claude-clone-refresh.sh` which rebuilds every mirror's clone. A `/tmp/claude-multiacct-refresh.lock` flock (or mkdir fallback) serializes overlapping fires so two ditto passes never race over the same clone bundle.

3. **`com.user.claude-primary-patch-refresh`** — `WatchPaths` on the SAME `/Applications/Claude.app/Contents/Info.plist`. Because the loose-DR re-sign keeps the primary Squirrel-updatable, updates now actually apply — each drop restores a fresh Anthropic-signed pristine. On mtime change this agent invokes `bin/claude-primary-patch-refresh.sh`, which deletes the stale pristine snapshots (so the patch pipeline captures the NEW pristine) and re-runs `claude-multiacct primary-patch`. It shares the `/tmp/claude-multiacct-refresh.lock` with clone-refresh and completes FIRST, so clone-refresh always ditto's the already-patched primary.
...
`claude-multiacct doctor` reports the loaded state of all four agents. `claude-multiacct repair` re-renders and re-installs all four plists.
