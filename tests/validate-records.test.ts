...
// This suite runs on Node's built-in runner (`node --test`), NOT vitest. The
// oxlint config enables the `vitest` plugin repo-wide; two of its rules assume a
// vitest suite and misfire here: `no-import-node-test` (we DO use node:test by
// design) and `no-conditional-in-test` (it flags the `&&` inside `.some()` /
// `.filter()` predicate callbacks as branching test logic, which it is not).
// Both are false positives for this runtime, so they are disabled file-wide.
/* oxlint-disable vitest/no-import-node-test, vitest/no-conditional-in-test */
...
test("baseline valid registry produces ZERO errors (the spine is legal end-to-end)", async () => {
	...
	const { errors, cleanup } = await runFixture([
		validPrd,
		{
			...
			frontmatter: fm({ id: "PDR-0001", type: "PDR", references: ["PRD-0001"], supersedes: null }),
		},
		validAdr,
		{
...
test("DOMAIN_UNKNOWN's allowed set is sourced from owners.yaml, not hardcoded", async () => {
	...
	assert.deepEqual(
		rules(errors, "prd-0001"),
		[],
		"the governance record (in the set) must stay clean",
	);
});
