// Integration tests for the editorconfig gate (scripts/qa/editorconfig-check.ts).
//
...
//
// Unlike the other check scripts, editorconfig-check binds ROOT to the REAL repo
// via `import.meta.dirname` (not cwd), so a chdir fixture can't redirect it.
// Instead `node:child_process` is mocked so `spawnSync` returns controlled `ec`
// outputs — that is the only way to drive the three outcomes deterministically:
...
const MODULE = "../../scripts/qa/editorconfig-check.ts";
...
describe("editorconfig-check", () => {
	...
	it("invokes `ec` through the repo-scoped mise wrapper", async () => {
		await runWrapper({ status: 0, stdout: "", stderr: "" });
		expect(spawnSyncMock).toHaveBeenCalledTimes(1);
		const [call] = spawnSyncMock.mock.calls;
		const bin = String(call?.[0]);
		const argv = call?.[1] as string[];
		expect(bin.endsWith("bin/mise")).toBe(true);
		expect(argv).toEqual(["exec", "--", "ec"]);
	});
