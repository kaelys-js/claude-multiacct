// Extended coverage for the record validator (`scripts/validate-records.ts`).
//
...
const VR = fileURLToPath(new URL("../scripts/validate-records.ts", import.meta.url));
...
async function prdSections(): Promise<string[]> {
	const text = await readFile(join("templates", "prd.template.md"), "utf8");
...
async function runCli(args: string[]): Promise<{
	...
	try {
		await import("../scripts/validate-records.ts");
	} catch (error) {
...
async function withRejectingReadFile(
	...
	const mod = await import("../scripts/validate-records.ts");
	...
	it("throws when the on-disk schema cannot be loaded", async () => {
		...
		try {
			await expect(mod.validateRecords(root)).rejects.toMatchObject({
				operational: true,
				message: expect.stringContaining("cannot load schema/record.schema.json"),
			});
		} finally {
			await cleanup();
