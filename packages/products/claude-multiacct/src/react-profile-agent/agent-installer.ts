 ...
 * react-profile environment agent.
 *
 * Shape mirrors `active-token-agent/agent-installer.ts` and the other agent
 * installers deliberately: a `LaunchctlPort` + narrow `AgentFsPort` are
 * injected, `installAgent` / `uninstallAgent` are BOTH gated on
 * `CLAUDE_MULTIACCT_ENABLE_SHIM=1` (or `{overrideFlag:true}` / `{flag:true}` for
 * callers), and `statusAgent` is a read-only inspection that ignores the flag.
 * Every mutating op snapshots the existing plist to
 * `~/.claude-multiacct-backups/<ts>/` before writing.
 *
 * Idempotence contract identical to the other agents: a second `installAgent`
 * with a byte-identical body is a no-op; a changed body triggers snapshot +
 * rewrite + bootout+bootstrap.
 *
 * `LaunchctlPort` + `AgentFsPort` are re-declared here rather than imported from
 * a sibling installer, matching the existing convention (each agent's ports
 * evolve independently of the others'). The agent is bootstrapped into
 * `gui/<uid>` with NO `SessionCreate` in its plist, so it lands in the user's
 * aqua session where its `launchctl setenv REACT_PROFILE 1` reaches
 * dock-launched apps — see `launchd-plist.ts`.
 *
 * @module
 */
...
import { REACT_PROFILE_LABEL } from "./launchd-plist.ts";
...
/**
 * Real `LaunchctlPort` binding, injectable `execFile` for tests.
 *
...
type BaseOpts = {
	 ...
	 * fall back to `overrideFlag` / env-var. Shared contract, mirrors
	 * `cli-shim/installer.ts` MutateOptions.
	 */
	flag?: boolean;
	log?: (m: string) => void;
};
...
function plistPathFor(plistDir: string): string {
	return join(plistDir, `${REACT_PROFILE_LABEL}.plist`);
}

/**
 * Install (or reload) the react-profile environment agent. See module docstring
 * for the idempotency + snapshot rules.
 *
...
export async function installAgent(opts: InstallAgentOpts): Promise<InstallAgentResult> {
	...
	if (alreadyPresent) {
		...
		await opts.fs.copyFile(plistPath, join(backupDir, `${REACT_PROFILE_LABEL}.plist`));
		await opts.fs.writeFile(plistPath, opts.plistBody);
		try {
			await opts.launchctl.bootout(opts.uid, REACT_PROFILE_LABEL);
		} catch {
...
/**
 * Uninstall the react-profile environment agent. Snapshots first, then bootout
 * + rm. Note: `launchctl setenv` writes are not reverted here — the variable
 * clears on the next logout/reboot, and removing the agent stops it being
 * re-set at the following login.
 *
...
export async function statusAgent(opts: StatusAgentOpts): Promise<StatusAgentResult> {
	...
	const { loaded } = await opts.launchctl.print(opts.uid, REACT_PROFILE_LABEL);
