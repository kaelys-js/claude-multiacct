/**
 * Intent: the react-profile agent installer stays GATED, exactly like the
 * watcher/daemon/active-token installers. `installAgent` and `uninstallAgent`
 * must be hard no-ops unless CLAUDE_MULTIACCT_ENABLE_SHIM=1 (or
 * `{overrideFlag:true}` / `{flag:true}`). The flag-off cases are the load-bearing
 * tripwires — bypass the gate on either and the matching test flips red.
 *
 * The idempotency + snapshot + reload cases pin the mechanics: unchanged body →
 * no-op, changed body → snapshot + rewrite + bootout+bootstrap. The real-body
 * case cross-checks the plist we install carries NO `SessionCreate` (which would
 * move the setenv into an isolated session where dock apps never see the
 * variable — the one thing this component exists to avoid).
 */
...
const plistPath = join(plistDir, `${REACT_PROFILE_LABEL}.plist`);
const uid = 501;
...
describe("installAgent — mechanics", () => {
	...
	it("second install with CHANGED body → snapshot + rewrite + bootout+bootstrap", async () => {
		...
		expect(lc.bootout).toHaveBeenCalledWith(uid, REACT_PROFILE_LABEL);
		expect(lc.bootstrap).toHaveBeenCalledWith(uid, plistPath);
		const okResult = result as Extract<typeof result, { skipped: false }>;
		const backupDir = okResult.backup as string;
		expect(fs.files.get(join(backupDir, `${REACT_PROFILE_LABEL}.plist`))).toBe(body);
	});
	...
	it("uses the real react-profile plist body — NO SessionCreate, NO WatchPaths (add either → RED)", async () => {
		const realBody = renderReactProfilePlist({
			label: REACT_PROFILE_LABEL,
			programArgs: ["/bin/launchctl", "setenv", "REACT_PROFILE", "1"],
			stdoutPath: "/tmp/react-profile.out",
			stderrPath: "/tmp/react-profile.err",
		});
		await installAgent({
			launchctl: lc,
			fs: fs.port,
		...
		expect(written).not.toContain("SessionCreate");
		expect(written).not.toContain("WatchPaths");
		expect(written).toContain("<string>REACT_PROFILE</string>");
	});
});
...
describe("uninstallAgent — flag gate + mechanics", () => {
	...
	it("plist present → snapshots, boots out, removes file", async () => {
		...
		expect(lc.bootout).toHaveBeenCalledWith(uid, REACT_PROFILE_LABEL);
		const okResult = result as Extract<typeof result, { skipped: false }>;
		expect(fs.files.get(join(okResult.backup as string, `${REACT_PROFILE_LABEL}.plist`))).toBe(
			body,
		);
	});
...
describe("real port factories", () => {
	...
	it("nodeAgentFsPort read/write/mkdir/rm/copyFile round-trip against a real tmpdir", async () => {
		...
		const root = await mkdtemp(`${tmpdir()}/cma-reactprofilefs-`);
		const port = nodeAgentFsPort();
	...
	it("nodeLaunchctlPort exposes the LaunchctlPort shape (print of missing agent → loaded=false)", async () => {
		...
		const r = await port.print(uid, REACT_PROFILE_LABEL);
	...
	it("nodeLaunchctlPort.bootstrap/bootout call through to injected execFile", async () => {
		...
		await port.bootout(uid, REACT_PROFILE_LABEL);
...
describe("installAgent / uninstallAgent — CLI-authoritative {flag} param", () => {
