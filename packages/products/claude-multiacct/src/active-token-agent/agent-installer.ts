 ...
 * gui-session active-token companion agent.
 *
 * Shape mirrors `launch/agent-installer.ts` and `watcher/agent-installer.ts`
 * deliberately: a `LaunchctlPort` + narrow `AgentFsPort` are injected,
 * `installAgent` / `uninstallAgent` are BOTH gated on
 * `CLAUDE_MULTIACCT_ENABLE_SHIM=1` (or `{overrideFlag:true}` for tests), and
 * `statusAgent` is a read-only inspection that ignores the flag. Every mutating
 * op snapshots the existing plist to `~/.claude-multiacct-backups/<ts>/` before
 * writing.
 *
 ...
 * aqua session where the keychain reads succeed — see `launchd-plist.ts`.
 *
 * @module
 */
...
import { ACTIVE_TOKEN_LABEL } from "./launchd-plist.ts";
...
function plistPathFor(plistDir: string): string {
	return join(plistDir, `${ACTIVE_TOKEN_LABEL}.plist`);
}

/**
 * Install (or reload) the active-token companion agent. See module docstring
...
export async function installAgent(opts: InstallAgentOpts): Promise<InstallAgentResult> {
	...
	if (alreadyPresent) {
		...
		await opts.fs.copyFile(plistPath, join(backupDir, `${ACTIVE_TOKEN_LABEL}.plist`));
		await opts.fs.writeFile(plistPath, opts.plistBody);
		try {
			await opts.launchctl.bootout(opts.uid, ACTIVE_TOKEN_LABEL);
		} catch {
...
/**
 * Uninstall the active-token companion agent. Snapshots first, then bootout + rm.
 *
...
export async function statusAgent(opts: StatusAgentOpts): Promise<StatusAgentResult> {
	...
	const { loaded } = await opts.launchctl.print(opts.uid, ACTIVE_TOKEN_LABEL);
