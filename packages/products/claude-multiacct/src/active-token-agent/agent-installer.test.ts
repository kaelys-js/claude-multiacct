/**
 * Intent: the active-token companion installer stays GATED, exactly like the
 * watcher/daemon installers. `installAgent` and `uninstallAgent` must be hard
 * no-ops unless CLAUDE_MULTIACCT_ENABLE_SHIM=1 (or `{overrideFlag:true}` /
 * `{flag:true}`). The flag-off cases are the load-bearing tripwires — bypass the
 * gate on either and the matching test flips red.
 *
 ...
 * case cross-checks the companion plist we install carries NO `SessionCreate`
 * (add it → the agent goes keychain-blind, which is the one thing this whole
...
const plistPath = join(plistDir, `${ACTIVE_TOKEN_LABEL}.plist`);
const uid = 501;
...
describe("installAgent — mechanics", () => {
	...
	it("second install with CHANGED body → snapshot + rewrite + bootout+bootstrap", async () => {
		...
		expect(lc.bootout).toHaveBeenCalledWith(uid, ACTIVE_TOKEN_LABEL);
		expect(lc.bootstrap).toHaveBeenCalledWith(uid, plistPath);
		const okResult = result as Extract<typeof result, { skipped: false }>;
		const backupDir = okResult.backup as string;
		expect(fs.files.get(join(backupDir, `${ACTIVE_TOKEN_LABEL}.plist`))).toBe(body);
	});
	...
	it("uses the real companion plist body — NO SessionCreate (add it → RED)", async () => {
		const realBody = renderActiveTokenPlist({
			label: ACTIVE_TOKEN_LABEL,
			watchedPath: "/Users/u/Library/Application Support/Claude/config.json",
			programArgs: ["/usr/bin/node", "/opt/active-token.js"],
			stdoutPath: "/tmp/active-token.out",
			stderrPath: "/tmp/active-token.err",
		});
		await installAgent({
			launchctl: lc,
			fs: fs.port,
		...
		expect(written).toContain("<key>WatchPaths</key>");
	});
});
...
describe("uninstallAgent — flag gate + mechanics", () => {
	...
	it("plist present → snapshots, boots out, removes file", async () => {
		...
		expect(lc.bootout).toHaveBeenCalledWith(uid, ACTIVE_TOKEN_LABEL);
		const okResult = result as Extract<typeof result, { skipped: false }>;
		expect(fs.files.get(join(okResult.backup as string, `${ACTIVE_TOKEN_LABEL}.plist`))).toBe(body);
	});
...
describe("real port factories", () => {
	...
	it("nodeAgentFsPort read/write/mkdir/rm/copyFile round-trip against a real tmpdir", async () => {
		...
		const root = await mkdtemp(`${tmpdir()}/cma-activetokenfs-`);
		const port = nodeAgentFsPort();
	...
	it("nodeLaunchctlPort exposes the LaunchctlPort shape (print of missing agent → loaded=false)", async () => {
		...
		const r = await port.print(uid, ACTIVE_TOKEN_LABEL);
	...
	it("nodeLaunchctlPort.bootstrap/bootout call through to injected execFile", async () => {
		...
		await port.bootout(uid, ACTIVE_TOKEN_LABEL);
