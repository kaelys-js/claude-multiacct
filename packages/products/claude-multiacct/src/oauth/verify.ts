 ...
 * # Probe subcommand
 *
 * The stock Anthropic `claude` CLI (v2.x) does NOT expose a subcommand
 * that prints identity JSON — `usage` is not a real subcommand, only a
 * slash command inside an interactive session. So `verifyToken`'s
 * `probeArgs` default of `["usage"]` will produce a `malformed` result on
 * a real CLI. Callers that need identity from a token do NOT use this
 * verifier — the `bin/cma` bundle passes a `realVerify` that runs
 * `claude -p ... hi` in --print mode (exit 0 = token works) and mints a
 * fresh local account uuid via `crypto.randomUUID`. That's an honest
 * local pool identifier — Anthropic never sees it. This module stays for
 * an eventual CLI that DOES expose an identity probe; the injected `exec`
 * + `probeArgs` shape keeps it easy to swap in when that lands.
 *
...
export type VerifyOptions = {
	...
	timeoutMs?: number;
	/**
	 * Override the probe argv. Defaults to `["usage"]` — a placeholder that
	 * no real Anthropic CLI subcommand honors (see module docstring). The
	 * `bin/cma` bundle bypasses this module entirely with its own
	 * exit-code-only verifier; keep the injection surface for a future CLI
	 * that DOES expose an identity probe.
	 */
