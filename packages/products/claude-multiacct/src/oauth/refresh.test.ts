...
describe("refreshTokens — endpoint required", () => {
	it("throws when endpoint is empty (TODO(pr5) gap — caller must supply)", async () => {
		const { fetchImpl } = makeFetch({ ok: true, status: 200, body: "{}" });
		await expect(() =>
			refreshTokens({ refreshToken: "R", fetchImpl, endpoint: "" }),
		).rejects.toThrow(/endpoint is required/u);
	});
});
