 ...
 * launchd agent that keeps the bridge daemon alive.
 *
 * Shape mirrors PR3's `watcher/agent-installer.ts`: a `LaunchctlPort` +
 * narrow `AgentFsPort` are injected, `installAgent` / `uninstallAgent`
 * are BOTH gated on `CLAUDE_MULTIACCT_ENABLE_SHIM=1` (or
 * `{overrideFlag:true}` for tests), and `statusAgent` is a read-only
 * inspection that ignores the flag. Every mutating op snapshots the
 * existing plist to `~/.claude-multiacct-backups/<ts>/` before writing.
 *
 * Idempotence contract identical to the watcher: a second `installAgent`
 * with a byte-identical body is a no-op; a changed body triggers
 * snapshot + rewrite + bootout+bootstrap.
 *
 * `LaunchctlPort` + `AgentFsPort` shapes are re-declared here rather than
 * imported from the watcher module so this file's tests aren't coupled
 * to the watcher's; two agents with different lifecycles benefit from
 * independent evolutions of their ports.
 *
 * @module
 */
...
import { DAEMON_LABEL } from "./launchd-plist.ts";
...
function plistPathFor(plistDir: string): string {
	return join(plistDir, `${DAEMON_LABEL}.plist`);
}

/**
 * Install (or reload) the bridge daemon's launchd agent. See module
 * docstring for the idempotency + snapshot rules.
 *
...
export async function installAgent(opts: InstallAgentOpts): Promise<InstallAgentResult> {
	...
	if (alreadyPresent) {
		...
		await opts.fs.copyFile(plistPath, join(backupDir, `${DAEMON_LABEL}.plist`));
		await opts.fs.writeFile(plistPath, opts.plistBody);
		try {
			await opts.launchctl.bootout(opts.uid, DAEMON_LABEL);
		} catch {
...
/**
 * Uninstall the daemon launchd agent. Snapshots first, then bootout + rm.
 *
...
export async function statusAgent(opts: StatusAgentOpts): Promise<StatusAgentResult> {
	...
	const { loaded } = await opts.launchctl.print(opts.uid, DAEMON_LABEL);
