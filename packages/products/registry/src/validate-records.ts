...
//
// Run natively by Node 26 (`node packages/products/registry/src/validate-records.ts`) — erasable TS
...
function isOperationalFault(e: unknown): e is OperationalFault {
	return (
		typeof e === "object" && e !== null && (e as { operational?: unknown }).operational === true
	);
}
...
// The @foundation/registry package root (this module lives at <pkg>/src/). Assets
...
async function loadOwners(root: string): Promise<OwnersManifest> {
	...
	try {
		...
		throw {
			operational: true,
			message: `cannot read owners.yaml at ${ownersPath}: ${message}`,
		} satisfies OperationalFault;
	}
	let parsed: unknown;
	try {
		...
		throw {
			operational: true,
			message: `cannot parse owners.yaml at ${ownersPath}: ${message}`,
		} satisfies OperationalFault;
	}
	...
	if (!Array.isArray(branches)) {
		throw {
			operational: true,
			message: `owners.yaml at ${ownersPath} has no 'branches' array`,
		} satisfies OperationalFault;
	}
	...
	if (domains.size === 0) {
		throw {
			operational: true,
			message: `owners.yaml at ${ownersPath} declared no branch keys`,
		} satisfies OperationalFault;
	}
	...
	for (const line of markdown.split("\n")) {
		...
		if (/^\s*(```|~~~)/u.test(line)) {
			...
			if (m && m[1] !== undefined && !line.startsWith("###")) {
				...
				if (stopAt !== undefined && heading.toLowerCase() === stopAt) {
					break;
				}
				out.push(heading);
			}
...
async function loadCanonicalSections(root: string): Promise<Map<string, string[]>> {
	...
	// Read every template in parallel (Promise.all) rather than awaiting each
	// sequentially — order-independent since each entry is keyed by its type; the
	// first rejecting read still propagates as the operational fault.
	const entries = await Promise.all(
		BODY_CHECK_TYPES.map(async (type): Promise<[string, string[]]> => {
			...
			try {
				...
				throw {
					operational: true,
					message: `cannot read template ${templatePath}: ${message}`,
				} satisfies OperationalFault;
			}
			// A template's frontmatter has no H2s; parse the body after it. Fall back
			// to the whole file if gray-matter can't split (still only H2s matter).
			let body = text;
			try {
			...
			if (headings.length === 0) {
				throw {
					operational: true,
					message: `template ${templatePath} declared no canonical '## ' sections`,
				} satisfies OperationalFault;
			}
...
// The mutable collections + read-only lookups the pass-1 per-record step shares
// with its caller. Extracting the per-record body into processRecord (below)
// lets each early-exit guard `return` instead of `continue` (no-continue),
// preserving the exact iteration and short-circuit behaviour.
type Pass1Context = {
	readonly errors: RecordError[];
	readonly model: Map<string, RecordNode>;
	readonly idToPaths: Map<string, string[]>;
	readonly root: string;
	readonly allowedDomains: Set<string>;
	readonly canonicalSections: Map<string, string[]>;
	readonly validate: ValidateFunction;
};

// Pass-1 processing for ONE record file: domain check, frontmatter parse, schema
// + filename + id-pattern + body-sections checks, and model insertion. Pushes
// every finding into ctx.errors and indexes the record in ctx.model /
// ctx.idToPaths. Early-`return`s replace the former per-iteration `continue`s;
// order and side effects are identical to the inline loop body.
function processRecord(ctx: Pass1Context, path: string, raw: string): void {
	const { errors, model, idToPaths, root, allowedDomains, canonicalSections, validate } = ctx;
	...
	if (domain === undefined) {
		errors.push({
			path,
			rule: "DOMAIN_UNKNOWN",
			message: `${path} is not under a governed domain directory (records/<domain>/…)`,
		});
	} else if (!domainGoverned) {
		errors.push({
			path,
			rule: "DOMAIN_UNKNOWN",
			message: `${path} is not a governed domain (${domain} is not a branch key in owners.yaml)`,
		});
	}
	...
	try {
		...
		errors.push({
			...
			message: `frontmatter YAML failed to parse: ${message}`,
		});
		return;
	}
	...
	if (!isPlainObject(data) || Object.keys(data).length === 0) {
		errors.push({
			path,
			rule: "FRONTMATTER_MISSING",
			message: "no frontmatter, or frontmatter is not a mapping",
		});
		return;
	}
	...
	if (!ok && validate.errors) {
		for (const e of validate.errors) {
			errors.push({
				...
				recordId: id,
				rule: "SCHEMA_INVALID",
				message: schemaErrorToMessage(e),
			});
		}
	}
	...
	if (id !== undefined && !/^(PRD|PDR|ADR|SPEC)-[0-9]{4}$/u.test(id)) {
		errors.push({
			path,
			recordId: id,
			rule: "ID_PATTERN",
	...
	const supersededBy =
		typeof data["supersededBy"] === "string" ? (data["supersededBy"] as string) : null;
...
export async function validateRecords(root: string): Promise<RecordError[]> {
	...
	try {
		...
		throw {
			operational: true,
			message: `cannot load ${SCHEMA_PATH}: ${message}`,
		} satisfies OperationalFault;
	}
	...
	try {
		...
		if (!st.isDirectory()) {
			throw {
				operational: true,
				message: `root is not a directory: ${root}`,
			} satisfies OperationalFault;
		}
	} catch (error) {
		...
		throw {
			operational: true,
			message: `cannot access root directory ${root}: ${message}`,
		} satisfies OperationalFault;
	}
	...
	try {
		...
		throw {
			operational: true,
			message: `cannot glob ${root}/**/*.md: ${message}`,
		} satisfies OperationalFault;
	}
	...
	// Read every record up front, in parallel (Promise.all) rather than awaiting
	// each read sequentially inside the loop. The per-file processing below stays
	// sequential over the sorted `files`, so error/model ordering is unchanged.
	let rawByPath: Map<string, string>;
	try {
		const texts = await Promise.all(files.map((path) => readFile(path, "utf8")));
		rawByPath = new Map(files.map((path, i) => [path, texts[i] ?? ""]));
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		throw {
			operational: true,
			message: `cannot read a record file: ${message}`,
		} satisfies OperationalFault;
	}

	const pass1: Pass1Context = {
		errors,
		model,
		idToPaths,
		root,
	...
	for (const path of files) {
		processRecord(pass1, path, rawByPath.get(path) ?? "");
	}
	...
	for (const node of model.values()) {
		...
		for (const ref of node.references) {
			const target = ref === self ? undefined : model.get(ref);
			if (ref === self) {
				errors.push({
					path: node.path,
					recordId: self,
					rule: "REF_SELF",
					message: `${self} references itself`,
				});
			} else if (!target) {
				errors.push({
					path: node.path,
					recordId: self,
					rule: "REF_UNRESOLVED",
					message: `${self} references ${ref} which does not resolve to any record`,
				});
			} else if (target.status === "rejected") {
				errors.push({
					path: node.path,
					recordId: self,
					rule: "REF_TO_REJECTED",
					message: `${self} references ${ref} whose status is 'rejected' (a rejected record was never in force)`,
				});
			}
		}
		...
		if (rule) {
			...
			// Self-refs (REF_SELF) and unresolved refs (REF_UNRESOLVED) are already
			// reported above, so only resolvable non-self parents are type-checked.
			for (const ref of node.references) {
				const target = ref === self ? undefined : model.get(ref);
				if (target && !rule.allowed.includes(target.type)) {
					const allowedStr = rule.allowed.length > 0 ? rule.allowed.join("|") : "(none — root)";
					errors.push({
						path: node.path,
						recordId: self,
						rule: "PARENT_TYPE",
			...
			if (target) {
				if (target.type !== node.type) {
					errors.push({
						...
						recordId: self,
						rule: "SUPERSEDE_TYPE",
						message: `${self} (type ${node.type}) supersedes ${target.id} (type ${target.type}); supersession is same-type only`,
					});
				}
				if (target.status !== "superseded") {
					errors.push({
						path: node.path,
						recordId: self,
						rule: "SUPERSEDE_STATUS",
						message: `${self} supersedes ${target.id} whose status is '${target.status}', but a superseded target must have status 'superseded'`,
					});
				}
				if (target.supersededBy !== self) {
					errors.push({
						path: node.path,
						recordId: self,
						rule: "SUPERSEDE_BACKLINK",
						message: `${target.id}.supersededBy is '${target.supersededBy ?? "null"}' but must be '${self}' (bidirectional supersession link is broken)`,
					});
				}
				...
				if (who) {
					...
					supersededByWho.set(node.supersedes, [self]);
				}
			} else {
				errors.push({
					path: node.path,
					recordId: self,
					rule: "REF_UNRESOLVED",
					message: `${self} supersedes ${node.supersedes} which does not resolve to any record`,
				});
			}
		}
	}
	...
	for (const [targetId, superseders] of supersededByWho) {
		if (superseders.length > 1) {
			const sorted = [...superseders].toSorted();
	...
	// Only records that actually declare a supersedes edge start a walk.
	for (const start of model.values()) {
		if (start.supersedes !== null) {
...
function parseArgs(args: string[]): CliOptions {
	let root = join(PACKAGE_ROOT, "records");
	let json = false;
	for (let i = 0; i < args.length; i++) {
		...
		if (a === "--json") {
			...
			if (next === undefined) {
				throw {
					operational: true,
					message: "--root requires a directory argument",
				} satisfies OperationalFault;
			}
			root = next;
			i++;
		} else if (a !== undefined && a.startsWith("--root=")) {
...
async function main(): Promise<number> {
	...
	try {
		...
		if (isOperationalFault(error)) {
			stderr.write(`validate-records: ${error.message}\n`);
			return 2;
		}
		throw error;
	}
	...
	try {
		...
		if (isOperationalFault(error)) {
			stderr.write(`validate-records: operational fault: ${error.message}\n`);
			return 2;
		}
...
const isMain = ((): boolean => {
	...
	try {
		return import.meta.filename === entry;
	} catch {
		return false;
	}
})();
