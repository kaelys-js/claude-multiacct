...
//
// Runs on vitest. Each test builds a throwaway fixture registry in os.tmpdir(),
// runs the PURE core `validateRecords(root)`, asserts the exact rule code(s),
// and cleans up.
//
...
// inside `.some()` / `.filter()` predicate callbacks as branching test logic,
// which it is not — a false positive here, so it is disabled file-wide.
...
// (../templates), resolved via import.meta.dirname so the read is cwd-independent.
const TEMPLATES_DIR = join(import.meta.dirname, "..", "templates");

// The four record types whose templates drive body-section validation.
const TEMPLATE_TYPES = ["prd", "pdr", "adr", "spec"];

// Derive each type's canonical H2 headings from the package's real templates —
// SAME single source the validator uses — so a valid fixture body is generated
// from the templates and can never drift from what the validator requires.
...
async function canonicalSections(): Promise<Map<string, string[]>> {
	...
	for (const t of TEMPLATE_TYPES) {
		const text = await readFile(join(TEMPLATES_DIR, `${t}.template.md`), "utf8");
...
async function canonicalBody(type: string | undefined): Promise<string> {
	...
	const canonical = await canonicalSections();
	const sections = canonical.get(type);
	if (!sections) {
		return "# Record body\n";
	}
	return [
...
function toYaml(f: Frontmatter, body: string): string {
	...
	if (f.date !== undefined) {
		put("date", f.date);
	} // caller controls quoting
	if (f.references !== undefined) {
		if (f.references.length === 0) {
			put("references", "[]");
		} else {
			lines.push("references:", ...f.references.map((r) => `  - ${r}`));
		}
	}
	if ("supersedes" in f) {
		put("supersedes", f.supersedes === null ? "null" : String(f.supersedes));
	}
	if ("supersededBy" in f) {
		put("supersededBy", f.supersededBy === null ? "null" : String(f.supersededBy));
	}
	lines.push("---", "", body);
	return lines.join("\n");
}
...
async function runFixture(
	files: FileSpec[],
	keys: string[] = DOMAIN_KEYS,
): Promise<{ errors: RecordError[]; cleanup: () => Promise<void> }> {
	...
	for (const t of TEMPLATE_TYPES) {
		await copyFile(join(TEMPLATES_DIR, `${t}.template.md`), join(templatesDir, `${t}.template.md`));
	}
...
describe("validateRecords", () => {
	it("baseline valid registry produces ZERO errors (the spine is legal end-to-end)", async () => {
		...
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "governance/pdr-0001-static-index.md",
				frontmatter: fm({
					id: "PDR-0001",
					type: "PDR",
					references: ["PRD-0001"],
			...
			validAdr,
			{
				rel: "governance/spec-0001-capabilities.md",
				frontmatter: fm({
					id: "SPEC-0001",
					type: "SPEC",
					references: ["ADR-0009", "PDR-0001"],
					supersedes: null,
				}),
			},
		]);
		await cleanup();
		expect(errors, `expected clean baseline, got: ${JSON.stringify(errors, null, 2)}`).toEqual([]);
	});

	// --- one negative fixture per rule code -------------------------------------

	it("SCHEMA_INVALID fires on a bad enum (status not in the lifecycle set)", async () => {
		...
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "build/adr-0001-orchestrator.md",
				frontmatter: fm({
					id: "ADR-0001",
					type: "ADR",
					owner: "@lead-build",
		...
		await cleanup();
		expect(
			rules(errors, "adr-0001").includes("SCHEMA_INVALID"),
			"bad status must be SCHEMA_INVALID",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("SCHEMA_INVALID fires on an UNQUOTED date coerced to a JS Date (enforces the quoted-date convention)", async () => {
		...
		expect(
			adrRules.includes("SCHEMA_INVALID"),
			"unquoted date must trip SCHEMA_INVALID (not silently coerced)",
		).toBe(true);
		expect(
		...
		expect(
			rules(errors, "prd-0001"),
			"a quoted-date PRD (as templates ship) must stay clean",
		).toEqual([]);
	});

	it("ID_DUPLICATE fires against BOTH files that declare the same id", async () => {
		...
		expect(dup.length, "ID_DUPLICATE must be emitted against both paths").toBe(2);
		expect(
			dup.some((e) => e.path.includes("adr-0001-one")) &&
				dup.some((e) => e.path.includes("adr-0001-two")),
			"both duplicate paths must be named",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("ID_PATTERN fires on a malformed id (subsumed by SCHEMA_INVALID; see report)", async () => {
		...
		expect(badRules.includes("ID_PATTERN"), "malformed id must trip ID_PATTERN").toBe(true);
		expect(
			badRules.includes("SCHEMA_INVALID"),
			"and SCHEMA_INVALID (the schema pattern is identical) — documented as subsumed",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("REF_UNRESOLVED fires when a reference names a record that does not exist", async () => {
		...
		await cleanup();
		expect(
			rules(errors, "adr-0001").includes("REF_UNRESOLVED"),
			"reference to a non-existent id must be REF_UNRESOLVED",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("REF_SELF fires when a record references its own id (PT-6)", async () => {
		...
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "governance/spec-0001-caps.md",
				frontmatter: fm({
					id: "SPEC-0001",
					type: "SPEC",
					references: ["SPEC-0001"],
					supersedes: null,
				}),
			},
		]);
		await cleanup();
		expect(rules(errors, "spec-0001").includes("REF_SELF"), "self-reference must be REF_SELF").toBe(
			true,
		);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("REF_TO_REJECTED fires when a parent's status is rejected (PT-7)", async () => {
		...
		const { errors, cleanup } = await runFixture([
			{
				rel: "governance/prd-0002-rejected-case.md",
				frontmatter: fm({
					id: "PRD-0002",
					type: "PRD",
					status: "rejected",
		...
		await cleanup();
		expect(
			rules(errors, "adr-0001").includes("REF_TO_REJECTED"),
			"referencing a rejected record must be REF_TO_REJECTED",
		).toBe(true);
		expect(
			rules(errors, "adr-0009-a-clean-sibling"),
			"the valid ADR sibling (cites PRD-0001) must stay clean",
		).toEqual([]);
	});

	it("GOLDEN: PARENT_TYPE fires when an ADR references a SPEC (the spine cannot invert)", async () => {
		...
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "governance/spec-0002-some-capability.md",
				frontmatter: fm({
					id: "SPEC-0002",
					type: "SPEC",
					references: ["ADR-0009"],
		...
		await cleanup();
		expect(
			rules(errors, "adr-0001").includes("PARENT_TYPE"),
			"ADR→SPEC must be PARENT_TYPE (spine can't invert)",
		).toBe(true);
		expect(
			rules(errors, "adr-0009-a-clean-sibling"),
			"the valid ADR sibling must stay clean",
		).toEqual([]);
	});

	it("GOLDEN: PARENT_COUNT fires when a SPEC cites only a PDR and no ADR (mustContain ADR)", async () => {
		...
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "governance/pdr-0001-index.md",
				frontmatter: fm({
					id: "PDR-0001",
					type: "PDR",
					references: ["PRD-0001"],
		...
		expect(
			specRules.includes("PARENT_COUNT"),
			"SPEC with no ADR parent must be PARENT_COUNT (mustContain ADR)",
		).toBe(true);
		expect(
			rules(errors, "adr-0009-a-clean-sibling"),
			"the valid ADR sibling must stay clean",
	...
	it("PARENT_COUNT fires when a PDR cites zero parents (needs exactly one PRD)", async () => {
		...
		const { errors, cleanup } = await runFixture([
			...
			{
				rel: "governance/pdr-0001-index.md",
				frontmatter: fm({ id: "PDR-0001", type: "PDR", references: [], supersedes: null }),
			},
		]);
		await cleanup();
		expect(
			rules(errors, "pdr-0001").includes("PARENT_COUNT"),
			"PDR with zero parents must be PARENT_COUNT",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("SUPERSEDE_TYPE fires when a record supersedes a DIFFERENT type", async () => {
		...
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "governance/pdr-0001-old.md",
				frontmatter: fm({
					id: "PDR-0001",
					type: "PDR",
					status: "superseded",
		...
		await cleanup();
		expect(
			rules(errors, "adr-0001-new").includes("SUPERSEDE_TYPE"),
			"cross-type supersession must be SUPERSEDE_TYPE",
		).toBe(true);
		expect(
			rules(errors, "adr-0009-a-clean-sibling"),
			"the valid ADR sibling must stay clean",
	...
	it("SUPERSEDE_STATUS fires when the superseded target is not marked superseded (SS-2a)", async () => {
		...
		await cleanup();
		expect(
			rules(errors, "adr-0002-new").includes("SUPERSEDE_STATUS"),
			"target not marked superseded must be SUPERSEDE_STATUS",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("GOLDEN: SUPERSEDE_BACKLINK fires when a superseded record lacks the supersededBy backlink (SS-2b)", async () => {
		...
		expect(
			newRules.includes("SUPERSEDE_BACKLINK"),
			"broken backlink must be SUPERSEDE_BACKLINK",
		).toBe(true);
		expect(
			!newRules.includes("SUPERSEDE_STATUS"),
			"SS-2a is satisfied here, so only the backlink should fail",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("SUPERSEDE_FORK fires when two live records supersede the same target (SS-3)", async () => {
		...
		expect(forkErrs.length >= 2, "both forking superseders must be flagged SUPERSEDE_FORK").toBe(
			true,
		);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("SUPERSEDE_CYCLE fires when supersedes forms a cycle (SS-4)", async () => {
		// WHY: a supersession chain is a history, not a loop. A→B→A means neither is
		// 'current' — an infinite regress the walker must catch.
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "build/adr-0001-a.md",
				frontmatter: fm({
					id: "ADR-0001",
					type: "ADR",
					owner: "@lead-build",
		...
		await cleanup();
		expect(
			errors.some((e) => e.rule === "SUPERSEDE_CYCLE"),
			"a supersedes cycle must be SUPERSEDE_CYCLE",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("FRONTMATTER_MISSING fires on a file with no frontmatter mapping", async () => {
		...
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "build/adr-0001-empty.md",
				frontmatter: {},
				raw: "# Just a heading, no frontmatter at all\n",
			},
		]);
		await cleanup();
		expect(
			rules(errors, "adr-0001-empty").includes("FRONTMATTER_MISSING"),
			"no-frontmatter file must be FRONTMATTER_MISSING",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("FILENAME_MISMATCH fires when the basename does not encode the id", async () => {
		...
		await cleanup();
		expect(
			rules(errors, "wrong-name-here").includes("FILENAME_MISMATCH"),
			"mismatched basename must be FILENAME_MISMATCH",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("FILENAME_MISMATCH fires when the slug is not lowercase-kebab (uppercase + space + punctuation)", async () => {
		...
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "build/adr-0001-Bad Slug!.md",
				frontmatter: fm({
					id: "ADR-0001",
					type: "ADR",
					owner: "@lead-build",
			...
			{
				rel: "build/adr-0002-clean-slug.md",
				frontmatter: fm({
					id: "ADR-0002",
					type: "ADR",
					owner: "@lead-build",
		...
		await cleanup();
		const badErrs = errors.filter(
			(e) => e.path.includes("Bad Slug") && e.rule === "FILENAME_MISMATCH",
		);
		expect(badErrs.length, "a malformed slug must be FILENAME_MISMATCH").toBe(1);
		expect(
			badErrs[0]?.message.includes("slug"),
			"the message must distinguish the slug-format failure",
		).toBe(true);
		expect(rules(errors, "adr-0002-clean-slug"), "the clean-slug sibling must pass").toEqual([]);
	});

	it("OWNER_DOMAIN_MISMATCH fires when a record's owner is not its domain's lead", async () => {
		...
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "backend/adr-0001-wrong-owner.md",
				frontmatter: fm({
					id: "ADR-0001",
					type: "ADR",
					owner: "@lead-design",
			...
			{
				rel: "backend/adr-0002-right-owner.md",
				frontmatter: fm({
					id: "ADR-0002",
					type: "ADR",
					owner: "@lead-backend",
		...
		expect(
			mismatch.includes("OWNER_DOMAIN_MISMATCH"),
			"owner != domain lead must be OWNER_DOMAIN_MISMATCH",
		).toBe(true);
		expect(
			rules(errors, "adr-0002-right-owner"),
			"the record owned by the real backend lead must stay clean",
		).toEqual([]);
		expect(
			rules(errors, "prd-0001"),
			"the governance PRD (owner = governance lead) must stay clean",
		).toEqual([]);
	});

	it("BODY_SECTIONS_MISSING fires when an ADR's body carries PDR sections instead of ADR sections", async () => {
		...
		const canonical = await canonicalSections();
		const pdrSections = canonical.get("PDR") ?? [];
		const adrSections = canonical.get("ADR") ?? [];
		const pdrBody = [
			"# Record body",
			"",
			...pdrSections.flatMap((h) => [`## ${h}`, "", "Placeholder.", ""]),
		].join("\n");
		const { errors, cleanup } = await runFixture([
			validPrd,
			{
				rel: "build/adr-0001-wrong-body.md",
				frontmatter: fm({
					id: "ADR-0001",
					type: "ADR",
					owner: "@lead-build",
			...
			{
				rel: "build/adr-0002-right-body.md",
				frontmatter: fm({
					id: "ADR-0002",
					type: "ADR",
					owner: "@lead-build",
		...
		const wrong = errors.filter(
			(e) => e.path.includes("adr-0001-wrong-body") && e.rule === "BODY_SECTIONS_MISSING",
		);
		expect(wrong.length, "an ADR with a PDR body must fire BODY_SECTIONS_MISSING").toBe(1);
		// The message must name at least one ADR heading absent from the PDR body.
		const missingAdrHeading = adrSections.find(
			(h) => !pdrSections.some((p) => p.toLowerCase() === h.toLowerCase()),
		);
		expect(
			missingAdrHeading !== undefined,
			"sanity: ADR and PDR templates must differ in at least one heading",
		).toBe(true);
		expect(
			wrong[0]?.message.toLowerCase().includes((missingAdrHeading ?? "").toLowerCase()),
			`message must name a missing ADR heading (${missingAdrHeading})`,
		).toBe(true);
		expect(
			rules(errors, "adr-0002-right-body"),
			"the ADR with the correct body must stay clean",
		).toEqual([]);
	});

	it("FILENAME_MISMATCH fires when the filename type disagrees with the frontmatter type (mislabeled record)", async () => {
		...
		await cleanup();
		expect(
			rules(errors, "build/adr-0001-x").includes("FILENAME_MISMATCH"),
			"a filename type that disagrees with the frontmatter id must be FILENAME_MISMATCH",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("SCHEMA_INVALID fires when the frontmatter id and type disagree internally (id ADR-0003, type PDR)", async () => {
		...
		const { errors, cleanup } = await runFixture([
			...
			{
				rel: "build/adr-0003-id-type-disagree.md",
				frontmatter: fm({
					id: "ADR-0003",
					type: "PDR",
					owner: "@lead-build",
		...
		await cleanup();
		expect(
			rules(errors, "adr-0003-id-type-disagree").includes("SCHEMA_INVALID"),
			"an id/type mismatch must be SCHEMA_INVALID",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("DOMAIN_UNKNOWN fires when a record sits under a non-governed domain directory", async () => {
		...
		const { errors, cleanup } = await runFixture([
			...
			{
				rel: "notadomain/adr-0001-orphan-domain.md",
				frontmatter: fm({
					id: "ADR-0001",
					type: "ADR",
					owner: "@lead-build",
		...
		expect(
			badRules.includes("DOMAIN_UNKNOWN"),
			"record under a non-governed domain must be DOMAIN_UNKNOWN",
		).toBe(true);
		expect(
			rules(errors, "prd-0001"),
			"the sibling under a valid domain dir must stay clean",
		).toEqual([]);
	});

	it("DOMAIN_UNKNOWN's allowed set is sourced from owners.yaml, not hardcoded", async () => {
		...
		await cleanup();
		expect(
			rules(errors, "build/adr-0001").includes("DOMAIN_UNKNOWN"),
			"a domain absent from THIS owners.yaml must be DOMAIN_UNKNOWN",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the governance record (in the set) must stay clean").toEqual(
			[],
		);
	});

	it("a record carrying a bare 'domain:' frontmatter key is now SCHEMA_INVALID (domain removed from schema)", async () => {
		...
		await cleanup();
		expect(
			rules(errors, "adr-0001-has-domain").includes("SCHEMA_INVALID"),
			"a stray domain: key must now be SCHEMA_INVALID",
		).toBe(true);
		expect(rules(errors, "prd-0001"), "the valid PRD sibling must stay clean").toEqual([]);
	});

	it("a plain PRD supersession chain (no cycle, correct backlink) is CLEAN", async () => {
		...
		const { errors, cleanup } = await runFixture([
			{
				rel: "governance/prd-0001-old.md",
				frontmatter: fm({
					id: "PRD-0001",
					type: "PRD",
					status: "superseded",
					references: [],
					supersededBy: "PRD-0002",
				}),
			},
			{
				rel: "governance/prd-0002-new.md",
				frontmatter: fm({ id: "PRD-0002", type: "PRD", references: [], supersedes: "PRD-0001" }),
			},
		]);
		await cleanup();
		expect(
			errors,
			`a legal supersession pair must be clean, got: ${JSON.stringify(errors, null, 2)}`,
		).toEqual([]);
	});
});
