// In-process coverage for `packages/shared/utils/sync/src/turbo.ts` — the codegen that derives the
...
describe("sync/turbo", () => {
	...
	it("emits the static test + coverage + hygiene tasks with their exact cache/output shape", async () => {
		...
		// qa:hooks busts on the root lefthook stub AND the moved base it extends.
		expect(tasks["qa:hooks"]).toEqual({
	...
	it("derives one lint:<tool> task per linting tool, scoped to that tool's globs + config", async () => {
		...
		// `lint:oxlint` didn't carry the oxlint config, editing that config wouldn't
		// re-run oxlint — a correctness hole. As of Stage-0 the oxlint config lives in
		// @foundation/config (reached by `--config`), so the input is the moved base.
		const tasks = await generateTasks();
		// ext-match tool → per-extension globs, sorted, plus its config file.
		expect(tasks["//#lint:oxlint"]?.["inputs"]).toContain("**/*.ts");
		expect(tasks["//#lint:oxlint"]?.["inputs"]).toContain("packages/shared/config/oxlint.json");
		...
		// gitleaks reaches its moved config by `--config` (no root stub), so the base
		// is its config input.
		expect(tasks["//#lint:gitleaks"]?.["inputs"]?.[0]).toBe("$TURBO_DEFAULT$");
		expect(tasks["//#lint:gitleaks"]?.["inputs"]).toContain("packages/shared/config/gitleaks.toml");
	...
	it("derives syncpack's lint + format tasks scoped to its config, and fans them into the aggregators", async () => {
		...
		// each carrying its config in inputs so editing the config busts only syncpack's
		// cache. As of Stage-0 that config lives in @foundation/config. And all three
		// must be in their umbrella aggregators or syncpack silently stops running in CI.
		const cfg = "packages/shared/config/syncpack.json";
		const tasks = await generateTasks();
		expect(tasks["//#lint:syncpack"]?.["inputs"]).toContain(cfg);
		expect(tasks["//#format:syncpack"]?.["cache"]).toBe(false);
		expect(tasks["//#format:syncpack"]?.["inputs"]).toContain(cfg);
		expect(tasks["//#format:check:syncpack"]?.["inputs"]).toContain(cfg);
		expect((tasks["//#qa:lint:all"] as { dependsOn: string[] }).dependsOn).toContain(
			"//#lint:syncpack",
		);
