// Coverage for `packages/shared/utils/sync/src/schemas.ts` — the vendored-schema refetcher.
//
...
import { reconcile, isReachable, type SchemaOutcome } from "../src/schemas.ts";
import { runScript, resetHarness } from "./harness.ts";

const SCHEMAS = fileURLToPath(new URL("../src/schemas.ts", import.meta.url));

// The 14 vendored schema basenames the script fetches (SCHEMA_SOURCES keys),
// in the sorted order the script emits them ("markdownlint" before "-cli2").
const SCHEMA_NAMES = [
	"commitlint",
	"github-workflow",
	"lefthook",
...
describe("sync/schemas — main() pipeline", () => {
	...
	it("--check passes (exit 0) and reports N/N verified when every vendored copy matches upstream", async () => {
		...
		expect(res.exitCode).toBeUndefined();
		expect(res.stdout).toContain(`Schemas are in sync (14/14 verified).`);
	});
	...
	it("time-bounds every upstream fetch with an abort signal so a hung endpoint can't stall the gate", async () => {
		// WHY: without a per-fetch timeout, one slow upstream hangs `Promise.all`
		// forever and the whole drift gate (pre-push/CI) stalls — strictly worse than
		// the unreachable-warning path this module promises. Pin that every fetch is
		// handed an AbortSignal so the time-bound can't be silently dropped.
		const seen: Array<AbortSignal | undefined> = [];
		const capturingFetch = ((_url: string, opts?: { signal?: AbortSignal }) => {
