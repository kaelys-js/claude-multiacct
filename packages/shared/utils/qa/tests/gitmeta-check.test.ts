// Integration tests for the git-metadata validator (packages/shared/utils/qa/src/gitmeta-check.ts).
//
...
const MODULE = "../src/gitmeta-check.ts";
...
const GOOD_GITIGNORE = [
	...
	"coverage/",
	"build/",
	"!packages/products/registry/records/build/",
	"!packages/products/registry/records/build/**",
	"!pnpm-lock.yaml",
	"!bin/mise",
	"",
...
function makeRepo(over: Overrides = {}): string {
	...
	// The record spine now lives in the @foundation/registry package (STEP-0.5),
	// so the fixture builds records/schema/templates/owners.yaml under that path —
	// exactly where gitmeta-check's MUST_NOT_IGNORE assertions look.
	const reg = join(dir, "packages", "products", "registry");
	for (const d of domains) {
		mkdirSync(join(reg, "records", d), { recursive: true });
		writeFileSync(join(reg, "records", d, "x.md"), "# x\n");
	}
	mkdirSync(join(reg, "schema"), { recursive: true });
	writeFileSync(join(reg, "schema", "record.schema.json"), "{}\n");
	mkdirSync(join(reg, "templates"), { recursive: true });
	writeFileSync(join(reg, "templates", "prd.template.md"), "# t\n");
