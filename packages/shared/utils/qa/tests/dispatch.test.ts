// Tests for the QA dispatcher (packages/shared/utils/qa/src/dispatch.ts).
//
// Two layers:
...
import { selectByExt, selectByRegex, expandArgv } from "../src/dispatch.ts";
import type * as DispatchModule from "../src/dispatch.ts";
import { FILES } from "../src/registry.ts";
...
async function importDispatch(root: string): Promise<typeof DispatchModule> {
	process.chdir(root);
	vi.resetModules();
	return await import("../src/dispatch.ts");
}
