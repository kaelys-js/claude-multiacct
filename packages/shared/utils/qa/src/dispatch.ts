...
// `stdio: "inherit"` streams the tool's output live (the shared miseExec default
// captures it; here we override so users see progress).
function mise(argv: string[], cwd = ROOT): boolean {
	return miseExec(argv, { cwd, stdio: "inherit" }).status === 0;
}
